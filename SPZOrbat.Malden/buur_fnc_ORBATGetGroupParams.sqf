/*
	changed by buur:
	added playerID to Function output definitions and return param indexS
	...
	buur:
	added playerID class to connect class to multiplayer class
*/
...
if (typename _class == typename "") exitwith {
	switch (tolower _class) do {
		...
		case "playerID":	{DEF_indexplayerID};
		default				{-1};
	};
};
...
// get playerID
_playerID = (gettext (_class >> "playerID"));


////////////////////////////////////////////////////////////////////////////////////////////////
//--- Background Texture
// here are the insigna defined
// following lines make error in singleplayer
_insignia = gettext (_class >> "insignia");
_insignia = [DEF_overrideinsignia,_insignia] call _fnc_checkOverride;

	if (!isnil _playerID)  then
		{ if (isPlayer (call compile _playerID))then
			{
				_insignia =	squadParams _playerIDVariable select 0 select 4;
				_insignia = [DEF_overrideinsignia,_insignia] call _fnc_checkOverride;
			};
		};
...
////////////////////////////////////////////////////////////////////////////////////////////////
//--- Commander
// Commander is now the multiplayer unit.
...
if (isnil _playerID) then {
	...
	if (isclass _cfgCommander) then {
	    ...
	    {
				_commander = name (call compile _playerID);
			};

////////////////////////////////////////////////////////////////////////////////////////////////
//--- Commander Rank (empty string will use unit size default)
// buur: Commander Rank is now adjusted to the multiplayer unit
...
if (!isnil _playerID) then
	{
		_commanderRank = rank (call compile _playerID);
		_commanderRank = [DEF_overrideCommanderRank,_commanderRank] call _fnc_checkOverride;
		_commanderRankNameShort = "";
		_commanderRankTexture = "";
		if (_commanderRank == "") then {_commanderRank = gettext (_cfgSize >> "commanderRank");}; //--- Apply rank based on unit size
...
_result set [DEF_indexplayerID, _playerID];
_result
