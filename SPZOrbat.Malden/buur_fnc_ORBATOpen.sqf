/*
changes by buur:
switch from BIS_fnc_ORBATGetGroupParams to buur_fnc_ORBATGetGroupParams
show assets when unit is not last in its tier
...
_class = _this param [0,configfile >> "CfgORBATDefault",[configfile]];
_tags = _this param [1,[],[[]]];
_deepnessLimit = _this param [2,-1,[0]];
_onClick = _this param [3,[],[[]]];
_parentDisplay = _this param [4,_parentDisplayDefault,[displaynull]];
...
BIS_fnc_ORBAT_loadConfig = {
	...
	_class = _this select 0;
	_classParams = _class call buur_fnc_ORBATGetGroupParams;
	_result = [];
	...
	if (_onClickID >= 0) then {
		...
		_onClickCode = _onCLickCode param [0,{},[{}]];
	...
	if (count _result > 0) then {
		_classParams set ["assetsText" call buur_fnc_ORBATGetGroupParams,""];
	};
	*/
...
BIS_fnc_ORBAT_insignia = (BIS_fnc_ORBAT_structure select 1) select ("insignia" call buur_fnc_ORBATGetGroupParams);
...
BIS_fnc_ORBAT_indexSubclassesCount =	("subClasses" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_indexTexture =		("texture" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_indexSize =		("size" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_indexColor =		("color" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_indexTextureSize =	("textureSize" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_indexInfoText =		("infoText" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_indexSizeTexture =	("sizeTexture" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_indexTextShort =		("textShort" call buur_fnc_ORBATGetGroupParams);
BIS_fnc_ORBAT_countIndex =		("count" call buur_fnc_ORBATGetGroupParams);
	...
	if !(BIS_fnc_ORBAT_mouseClickDisable) then {
		...
		if (count _selected > 0) then {
			...
			if (count _params > BIS_fnc_ORBAT_countIndex) then {
				...
				_close = _close param [0,false,[false]];
...
_footer ctrlsettext (_parentParams select ("text" call buur_fnc_ORBATGetGroupParams));
